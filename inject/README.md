...
`playlist-menu.js` targets Jellyfin 10.11.x. It replaces the item playlist dialog with a compact checked popover, exposes the same popover from a player bookmark button, and keeps the exact `Legacy playlist overlay` entry available for the native editor.
